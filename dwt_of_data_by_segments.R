...
#foreach can not work with sideeffects. One should make a separate code to take this into account    
    ...
    for(r in 1:N.of.sensors){ # we have N.of.sensors rows in data - one row for every sensor
        ...
        dd <- dwt(mat_temp[r,])
        MAT[ fullCase_cur_number, ( (r-1)*(2*w.lenght)+1 ) : ( (r-1)*(2*w.lenght) + w.lenght) ] <-
            unlist( dd@W ) 
        MAT[ fullCase_cur_number, ( (r-1)*(2*w.lenght)+w.length+1 ) : ( (r-1)*(2*w.lenght) + 2*w.lenght) ] <-
            unlist( dd@V ) 
...
# Save the data
library(MASS)
write.matrix(MAT, file = paste0("dwt_MAT_", folder, "_SegmentsSeparate.csv") , sep = ",")
